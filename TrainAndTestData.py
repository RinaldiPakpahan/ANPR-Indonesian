...
def main():
    ...
    imgGrayscale, imgThresh = Preprocess.preprocess(imgTestingNumbers)
    # imgThresh = cv2.resize(imgThresh, (0,0), fx = 1.5, fy = 1.5)
    # thresholdValue, imgThresh = cv2.threshold(imgThresh, 0.0, 255.0, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    # cv2.imshow("1", imgGrayscale)
    
    # imgGray = cv2.cvtColor(imgTestingNumbers, cv2.COLOR_BGR2GRAY)       # get grayscale image
    # imgGrayscale = np.invert(imgGray)
    # imgMaxContrastGrayscale = maximizeContrast(imgGrayscale)
    # height, width = imgGrayscale.shape
    # imgBlurred = np.zeros((height, width, 1), np.uint8)
    # imgBlurred = cv2.GaussianBlur(imgMaxContrastGrayscale, (5,5), 0)                    # blur
    ...
    #                                   19,                                   # size of a pixel neighborhood used to calculate threshold value
    #                                   9)                                    # constant subtracted from the mean or weighted mean
    cv2.imshow("1", imgGrayscale)
    ...
    for contourWithData in validContoursWithData:            # for each contour
        ...
        cv2.rectangle(imgTestingNumbers,                                        # draw rectangle on original testing image
                      ...
                      (0, 255, 0),              # green
                      2)                        # thickness
        print(contourWithData.intRectX)
