...
def main():
    ...
    while (loop):
        ...
        imgOriginalScene  = imutils.resize(frame, width = 800)
        # imgOriginalScene = cv2.rectangle(imgOriginalScene,((imgOriginalScene.shape[1]//2-230),(imgOriginalScene.shape[0]//2-80)),((imgOriginalScene.shape[1]//2+230),(imgOriginalScene.shape[0]//2+80)),SCALAR_GREEN,3)
        ...
        if count == (VERIF-1):
            if (license[VERIF-1] == ""):
                print("Tidak ada karakter yang dikenali\n")
            else:
        ...
        if (key == ord('s')):
            ...
            knn = int(knn)
            knn = knn + 1
        if (key == 27): # if the 'esc' key is pressed, stop the loop
            break
...
# end main
    ...
    # imgOriginalScene  = cv2.imread("71.png")               # open image
...
def searching(imgOriginalScene,loop):
    ...
    if len(listOfPossiblePlates) == 0:
        ...
        # if (loop == False):
            # cv2.imshow("imgPlate", licPlate.imgPlate)           # show crop of plate and threshold of plate
            # cv2.imshow("imgThresh", licPlate.imgThresh)
            # cv2.imwrite("crop_plat.png", licPlate.imgPlate)
        ...
        # writeLicensePlateCharsOnImage(imgOriginalScene, licPlate) #menampilkan plat nomor terdeteksi di gambar
        ...
        if (loop == True):
            ...
            # listOfPlat = ''.join(map(str, licPlate.strChars))
            listOfPlat = licPlate.strChars.split(' ')
            outfile.write(str(listOfPlat))
